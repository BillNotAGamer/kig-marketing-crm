...
## Phân quyền Google Drive Task Assets Phase 5 - 2026-09-19

Phase 5 triển khai phân quyền tài sản đính kèm (Deliverables / Task Assets) tuân thủ đúng baseline V1.1 đã duyệt:

- `asset:create-any`: HEAD có quyền đính kèm tài sản Google Drive hợp lệ vào bất kỳ công việc nào chưa xóa (`OPEN`, `COMPLETED`, `CANCELLED`).
- `asset:create-own`: DEPUTY và EMPLOYEE chỉ có quyền đính kèm tài sản Google Drive vào công việc `OPEN` chưa xóa đang được phân công cho chính mình (`task.assignedToId === actor.id && task.status === "OPEN" && task.deletedAt === null`). Không được đính kèm vào công việc của người khác hoặc công việc đã hoàn thành/bị hủy.
- `asset:read-team`: HEAD và DEPUTY đọc tất cả tài sản đang hoạt động của các công việc thuộc nhóm chưa bị xóa.
- `asset:read-own`: EMPLOYEE chỉ đọc tài sản của các công việc đang được phân công cho mình. Truy cập tài sản của công việc người khác trả về 404 (hoặc 403). Tài sản đã xóa mềm bị ẩn khỏi mọi truy vấn vận hành thông thường.
- `asset:delete-any`: HEAD có quyền gỡ bỏ (xóa mềm) bất kỳ tài sản đang hoạt động nào trên công việc chưa bị xóa.
- `asset:delete-own`: DEPUTY và EMPLOYEE chỉ được phép gỡ bỏ tài sản do chính mình tạo (`asset.createdById === actor.id`) trên công việc đang được phân công cho mình (`task.assignedToId === actor.id`) khi công việc đang ở trạng thái `OPEN` và chưa bị xóa. Không được gỡ tài sản do HEAD hoặc người khác đính kèm.
